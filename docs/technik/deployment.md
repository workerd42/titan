...
> 🔒 **Zugangsschutz (aktualisiert 2026-07-17): echtes Login-Gate LIVE.** Die frühere nginx-Basic-Auth wurde **entfernt**; Inhalte sind jetzt **serverseitig hinter der Anmeldung** geschützt (Better-Auth-Login-Gate, `invite-only`). Öffentlich erreichbar ist nur `/konto` (Login) + `/api/auth`; alle Lerninhalte leiten ohne Session auf `/konto`. Registrierung ist geschlossen (`disableSignUp`) — Accounts werden im **Admin-Panel** (`/admin`, nur `platform-admin`) angelegt; erster Admin per `scripts/bootstrap-admin.mjs` (im Prod-Image, nach der Migration). Der nginx-443-Block enthält die `auth_basic`-Zeilen nur noch **auskommentiert** (Backup `.bak-*` daneben). Details: [admin-panel.md](admin-panel.md), [ki-governance.md](../ki/ki-governance.md). *Hinweis: Postgres ist nur im Docker-Netz erreichbar; App lauscht auf `127.0.0.1:8080`.*
