...
**Implementiert (9 Modultypen, alle Stufe A, deterministisch):** `swot`, `smart`, `deckungsbeitrag`, `marktanteil`, `preisberechnung`, `vier-stufen`, `scoring`, **`portfolio` (BCG-/Portfolio-Matrix)**, **`breakeven` (Gewinnschwellen-Rechner)**. Das `werkzeug`-Enum (`src/content.config.ts`) führt exakt diese 9 Werte; **15 der 46 Planeten** haben ein Werkzeug zugewiesen (`swot` ×3, `smart` ×3, `scoring` ×3, je ×1 `deckungsbeitrag`/`marktanteil`/`preisberechnung`/`vier-stufen`/`portfolio`/`breakeven`). Fehlt einem Planeten das Werkzeug, zeigt die „Interaktive Module"-Kachel ehrlich „bald".
