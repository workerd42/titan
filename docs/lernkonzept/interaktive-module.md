...
**Implementiert (10 Modultypen, alle Stufe A, deterministisch):** `swot`, `smart`, `deckungsbeitrag`, `marktanteil`, `preisberechnung`, `vier-stufen`, `scoring`, `portfolio` (BCG-Matrix), `breakeven` (Gewinnschwelle), **`statistik` (Mittelwerte & Streuung — neu 2026-08)**. Das `werkzeug`-Enum (`src/content.config.ts`) führt exakt diese 10 Werte; **17 der 46 Planeten** haben ein Werkzeug (`swot` ×3, `smart` ×3, `scoring` ×4, je ×1 `deckungsbeitrag`/`marktanteil`/`preisberechnung`/`vier-stufen`/`portfolio`/`breakeven`/`statistik`).

> **Abdeckungs-Philosophie (2026-08, wichtig):** Ziel ist NICHT „46/46 haben einen Rechner". Interaktive Werkzeuge passen zu **quantitativen/strategischen** Themen; ein erzwungener Rechner auf einem **qualitativen** Thema (HB4-Führung/Kommunikation, HB1-Erhebungsmethoden, HB3-Qualität) wäre fachlich falsch (Design-Kompass). Für qualitative Themen ist die **Anwenden-Phase = Fallbeispiel + freies Antwortfeld + späteres KI-Fachgespräch (Phase 3)** die richtige Interaktion — kein „Dead End". Coverage wächst dort, wo ein Werkzeug echten fachlichen Mehrwert hat. Verbleibende sinnvolle Werkzeug-Kandidaten (neue Typen nötig): `kpi`/Kennzahlensystem (HB3 KPI-Systematik), `werbeerfolg` (HB3 Werbeerfolgskontrolle), `prozess-sequencer` (HB1 Marktforschungsprozess), `arbeitszeit-rechner` (HB4 Arbeitszeitmodelle).

> **2026-07-28 — `portfolio` + `breakeven` ergänzt** (aus der PDF-Methodenanalyse, höchstes Curriculum-Gewicht). `portfolio`: BCG-Matrix mit automatischer Einordnung (Marktwachstum × relativer Marktanteil, Schwellen 10 % / 1,0) + Normstrategien — Planet HB3 *Controllinginstrumente*. `breakeven`: Gewinnschwelle inkl. Zielgewinn-Menge — Planet HB2 *Produktpolitik*. Beide auch in der Directus-Werkzeug-Auswahl.
