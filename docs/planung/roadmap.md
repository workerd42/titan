...
- [ ] **Build-/Versions-Transparenz im Info-Panel (Seite 1):** aktuell Hardcode (Status jetzt korrigiert auf „Phase 2 · Beta", Version 0.2.0). Geplant, damit es nicht wieder veraltet: **echte Build-Identität injizieren** (Git-Short-SHA + Umgebung staging/prod via `deploy.sh`/Env, nicht aus dem Repo-Build fragil per `git`), **Versions-Bump-Disziplin je Phase**, und den Phase-Status aus **einer Quelle** ziehen (nicht mehrfach hardcoden). *(Auslöser 2026-07-23: Status stand fälschlich noch auf „Phase 1".)*
