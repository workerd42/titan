...
- [x] **Erster Live-Deploy mit Secrets** auf dem VPS (`.env` neben `docker-compose.yml`: `POSTGRES_*`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`) — **erfolgt 2026-07-15**. DB-Zugangsdaten werden als diskrete `PG*`-Felder übergeben (nicht als URL) — siehe Stolperstein in [deployment.md](../technik/deployment.md).
...
- [ ] **Off-site-Backup** + externes Monitoring (Anbieter EU/DE). Off-site-Ziel: **Hetzner Storage Box 1 TB** (DE), sobald geholt — IONOS scheidet vertraglich aus. Details in [deployment.md](../technik/deployment.md).
- [x] **Build-/Versions-Transparenz im Info-Panel (Seite 1)** (2026-07-31): **eine Quelle** für Version/Phase/Commit/Umgebung (`src/lib/build-info.ts`, Konstante `PHASE`); der Info-Panel liest nur noch von dort. **Build-Identität wird injiziert** — `deploy.sh` setzt `COMMIT_SHA` (Git-Short-SHA) + `DEPLOY_ENV`, Compose reicht sie als Build-Args in den Astro-Build (graceful Fallback „dev"/„local" ohne Deploy-Kontext). Kein mehrfaches Hardcoden mehr. *(Auslöser 2026-07-23: Status stand fälschlich noch auf „Phase 1".)*
...
- [x] Dashboard je Lernendem: **Gesamtfortschritt, Fortschritt je Handlungsbereich (HB1–HB4), Lernstreak, erledigte Themen, deck-reife Artefakte, letzte Aktivität** (Lerner-Cards; on-the-fly aus `user_progress`, `src/lib/progress-stats.ts`). *(Nebenbei behoben: `mergeProgress` bewahrt jetzt `artefakt` — vorher gingen Artefakte beim Server-Sync verloren.)*
...
- [x] **Nutzer-/Rollenverwaltung** (`/admin`, platform-admin-gated): Rollen setzen (platform-admin/org-admin/dozent/lerner), Ban/Unban, Nutzerliste — via Better-Auth-Admin-Plugin + Access-Control ([admin-panel.md](../technik/admin-panel.md), [rollen-rechte.md](../technik/rollen-rechte.md)).
...
- [ ] **Weitere ~21 Modultypen** (Rechner/Matrizen/Sequenzer/HB4) + Zuweisung der übrigen 33 Planeten — vollständiger Katalog in [interaktive-module.md](../lernkonzept/interaktive-module.md).
...
- [ ] **Kosmos-Authentizität à la NASA „Eyes on the Solar System" (Idee, terminiert 2026-07-23):** Design-/Motion-Techniken der NASA-Web-App **adaptieren, nicht kopieren** (keine Assets/Code, CSP `self`-only) — gefiltert durch die warme, ruhige Norive-Ästhetik (kein fotorealistischer NASA-Look). Betrifft die ersten drei Seiten (Universum/Galaxie/Planeten). **Zwei Stufen:**
  - **Stufe 1 — ohne WebGL, jetzt machbar (Kandidat für einen vorgezogenen „Kosmos-Politur"-Spike, ~M):** Parallax-Tiefen-Starfield ([starfield.ts](../../src/scripts/starfield.ts) erweitern), Bloom/Halo auf den SVG-Globes, faible Orbit-Ringe, cineastische GSAP-„Fly-to"-Transition beim Planet-Auswählen, fokus-/distanzbasiertes Label-Fading, langsame Eigenrotation. CSP-safe (GSAP ist lokal gebündelt), `prefers-reduced-motion`-Aus + Tastaturpfad Pflicht (0 axe).
  - **Stufe 2 — echtes WebGL (= dieser Abschnitt, three.js-Upgrade oben):** true 3D-Planeten, echtes Bloom-Post-Processing, Kamera. Bewusster Rendering-Wechsel, ~XL, mit a11y-Fallback.
  - **Rahmenbedingung (verbindlich, vom Gründer):** **Umsetzung reversibel halten** — alten Stand bewahren (Feature-Flag/Branch, klarer Rückführungspfad), der gesamte Prozess muss **valide** sein und **zur Gesamtarchitektur passen** (Astro-Hybrid, Vanilla-CSS/Norive-Tokens, Local-First, EU-Self-Hosting). Vor Rollout Machbarkeits-Spike auf *einer* Seite (Universum) bewerten. Feasibility-Analyse dazu im Chat/Notizen vom 2026-07-23.
