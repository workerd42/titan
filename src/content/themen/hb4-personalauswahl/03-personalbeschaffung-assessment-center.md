---
title: "Personalbeschaffung & Assessment-Center"
handlungsbereich: "hb4"
themengruppe: "Personalauswahl & Rekrutierung"
order: 3
description: "Interne versus externe Personalbeschaffung sowie das Assessment-Center als umfassendes, aber aufwendiges Auswahlverfahren für Führungspositionen."
merksatz: |
  Ein Assessment-Center zeigt, was ein Bewerber wirklich kann – ein Lebenslauf zeigt nur, was er behauptet zu können.
fallbeispiel:
  situation: |
    Die {{firma}} möchte kurzfristig einen Kapazitätsengpass in der
    Auftragsabwicklung überbrücken, ohne eine dauerhafte Neueinstellung vorzunehmen.
  aufgabe: |
    Schlagen Sie geeignete Maßnahmen der internen Personalbeschaffung vor.
  musterloesung: |
    Für einen kurzfristigen, nicht dauerhaften Engpass eignen sich Maßnahmen der internen
    Personalbeschaffung: die Anordnung von Überstunden, Urlaubsverschiebungen sowie der
    Einsatz interner kurzfristiger Aushilfen aus anderen Abteilungen. Diese Maßnahmen
    sind jedoch nicht für eine dauerhafte Lösung geeignet – sollte sich der
    Personalbedarf als langfristig herausstellen, wäre stattdessen die externe
    Personalbeschaffung (z. B. eine reguläre Stellenausschreibung) der passendere Weg.
pruefungsfrage:
  frage: |
    Die {{firma}} plant zusätzlich, künftig Führungspositionen über ein
    aufwendigeres Auswahlverfahren zu besetzen als bisher.
    a) Erläutern Sie den Grundgedanken und Nutzen eines Assessment-Centers.
    b) Nennen Sie einen Grund, warum Assessment-Center in der Praxis meist nur für
    Führungspositionen oder in verkürzter Form für Auszubildende eingesetzt werden.
    c) Grenzen Sie interne von externer Personalbeschaffung anhand je eines Beispiels ab.
  loesungsweg:
    - |
      Ein Assessment-Center (AC) ermöglicht es, mehrere Bewerber miteinander zu
      vergleichen und die Personalauswahl objektiver zu gestalten. Da Bewerber ihr Können
      in unterschiedlichen, realitätsnahen Situationen unter Beweis stellen müssen,
      lassen sich zudem mehrere Kompetenzfelder gleichzeitig erfassen, was mit einem
      reinen Vorstellungsgespräch nicht möglich wäre.
    - |
      Aufgrund des hohen Zeit-, Verwaltungs- und Kostenaufwands wird ein vollständiges
      Assessment-Center in der Praxis meist nur für Führungspositionen durchgeführt. Bei
      der Auswahl von Auszubildenden werden häufig verkürzte, sogenannte „kleine"
      Assessment-Center eingesetzt.
    - |
      Interne Personalbeschaffung: Besetzung durch Mitarbeiter aus den eigenen Reihen,
      z. B. durch interne Stellenausschreibung oder Versetzung. Externe
      Personalbeschaffung: Gewinnung neuer Mitarbeiter außerhalb des Unternehmens, z. B.
      über öffentliche Stellenanzeigen oder Personalvermittler.
wiederholungTage: 5
---

Bei der Personalbeschaffung wird grundsätzlich zwischen zwei Wegen unterschieden:

**Interne Personalbeschaffung** beinhaltet die Besetzung von Stellen durch Mitarbeiter aus den eigenen Reihen. Für kurzfristige, nicht dauerhafte Kapazitätsengpässe eignen sich dabei besonders die Anordnung von **Überstunden**, **Urlaubsverschiebungen** oder der Einsatz **interner kurzfristiger Aushilfen**. Für eine dauerhafte Stellenbesetzung ist die klassische interne Stellenausschreibung der übliche Weg.

**Externe Personalbeschaffung** gewinnt neue Mitarbeiter außerhalb des Unternehmens, etwa über öffentliche Stellenanzeigen, Personalvermittler oder Hochschulmarketing.

Für die eigentliche **Personalauswahl** unter mehreren Bewerbern hat sich für anspruchsvolle Positionen das **Assessment-Center (AC)** etabliert: ein strukturiertes Auswahlverfahren, bei dem mehrere Bewerber in unterschiedlichen, realitätsnahen Situationen (Rollenspiele, Präsentationen, Gruppenübungen) ihr Können unter Beweis stellen. Der Vorteil: Die Personalauswahl wird objektiver gestaltet, und es lassen sich mehrere Kompetenzfelder gleichzeitig erfassen, die ein einzelnes Vorstellungsgespräch nicht abbilden könnte.

**Prüfungsrelevanter Praxishinweis:** Aufgrund des hohen Zeit-, Verwaltungs- und Kostenaufwands wird ein vollständiges Assessment-Center in der Praxis meist nur für **Führungspositionen** durchgeführt. Für die Auswahl von Auszubildenden kommen häufig verkürzte, sogenannte **„kleine" Assessment-Center** zum Einsatz. Das Thema Assessment-Center ist in vielen Prüfungen immer wieder Bestandteil und sollte inklusive seiner Vor- und Nachteile sicher beherrscht werden.
